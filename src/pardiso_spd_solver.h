...
using VectorXr = Eigen::Matrix<double, -1 /* Dynamic */, 1>;
using MatrixXr = Eigen::Matrix<double, -1 /* Dynamic */, -1 /* Dynamic */>;
...
class PardisoSolver {
    ...
    virtual void Compute(const SparseMatrix& lhs, const std::map<std::string,double>& options);
...
class PardisoSpdSolver : PardisoSolver {
    ...
    void Compute(const SparseMatrix& lhs, const std::map<std::string, double>& options) override;
